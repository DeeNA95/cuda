...
template <typename T> __global__ void Zoomfrom (r_Ptr<T> a, r_Ptr<T> b, cr_Ptr<T> aold, int nx, int ny){
	...
	if(x>0 && x < nx-1 && y<ny-1)       a[idx(y,x)] = aold[mdx(y/2, x/2)]; // interior
	// top
	else if (y==0 && x>0 && x<nx-1)     a[idx(y,x)] = T(0); //cast 0 to type T
	else if (y == ny-1 && x>0 && x<nx-1)   a[idx(y,x)] = T(0); //bottom
	
	//sides
	else if (x==0 && y > 0 && y<ny-1)    a[idx(y,x)] = T(1);
	else if (x == nx-1 && y > 0 && y<ny-1)   a[idx(y,x)] = T(1);

	// corners
	else if (x ==0 && y==0) a[idx(y,x)] = T(0.5);
...
template <typename T> int cascade(int nx, int ny, int iter){
	...
	for (int mx = nx_start; mx<nx; mx*=2){
		...
		for( int k =0; k < iter/2; k++){
			stencil2d<T><<<blocks, threads>>> (dev_a.data().get(),
					dev_b.data().get(),
					mx, my
...
int main (int argc, char* argv[]){
	...
	if(type == 1) cascade<double>(ny,nx,iter_gpu);
	else cascade<float> (ny, nx, iter_gpu);
