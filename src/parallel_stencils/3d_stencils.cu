...
//one thread processes all points with a fixed x and y stepping though the full range of z
//
//add more threads to the kernel launch so that each point in the 3d grid is processed by a different thread

//v1
template <typename T> __global__ void stencil3d_1(cr_Ptr<T> a, r_Ptr<T> b, int nx, int ny, int nz){
	//nb the values incapsulated by the [] in a lambda are the capture clauses, they denote what external variables
	//the lambda can use within its scope, when paired with a & its a capture by reference so no copy overhead
	auto idx = [&nx, &ny](int z, int y, int x){ return (z*ny+y) * nx + x;}; //the global id in the 3d grid 
									      //
	int x = blockDim.x * blockIdx.x + threadIdx.x;
	int y = blockDim.y * blockIdx.y + threadIdx.y;
	//int z = blockDim.z * blockIdx.z + threadIdx.z;

	if ( x<1 || y<1 || x >= nx-1 || y > ny-1) return; // leave out edges because of stencil algo boundary

	for(int z=1; z<nz-1; z++) {
		b[idx(z,y,x)] = (T)(1.0/6.0) * (
...
int main(int argc, char* argv[]) {
    ...
    for(int i = 0; i < iterations; ++i) {
        stencil3d_1<float><<<blocks, threads>>>(d_in.data().get(), d_out.data().get(), nx, ny, nz);
    }
    ...
    tim.start();
    for (int i=0; i< iterations; ++i){
	    stencil3d_2<float><<<blocks_v2, threads_v2>>>(d_in.data().get(), d_out.data().get(), nx, ny, nz);
    }
    cudaDeviceSynchronize();
    tim.stop();
