...
__global__ void reduce5(r_Ptr<int> sums, cr_Ptr<int> data, int n) {
  ...
  int id = threadIdx.x;
  s[id] = 0;
  for (int tid = blockSize * blockIdx.x + threadIdx.x; tid < n;
  ...
  // below 32 will require sync warp over sync threads as all threads will be in
  // one warp
  ...
  if (id < 32) {
    ...
    if (id == 0)
      sums[blockIdx.x] = s[0]; // block sum in index 0 at global mem
  }
}
...
int main(int argc, char **argv) {
  ...
  // Warmup run
  reduce5<blockSize>
      <<<numBlocks, blockSize>>>(thrust::raw_pointer_cast(dev_sums.data()),
  ...
  dim3 numBlocks3D((nx + threadsPerBlock.x - 1) / threadsPerBlock.x,
  ...
  printf("   Grid: (%d, %d, %d), Block: (%d, %d, %d)\n", numBlocks3D.x,
         numBlocks3D.y, numBlocks3D.z, threadsPerBlock.x, threadsPerBlock.y,
         threadsPerBlock.z);
  printf("   Total threads: %d\n", numBlocks3D.x * numBlocks3D.y *
                                       numBlocks3D.z * threadsPerBlock.x *
  ...
  printf("         hierarchy through cg::this_grid() and "
