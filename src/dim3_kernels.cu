...
__global__ void grid3D(int nx, int ny, int nz,
                       int id) // array dims and id of thread to print info
{
  int x = blockIdx.x * blockDim.x +
          threadIdx.x; // find x using the index of the block * nthreads per
                       // block + thread index within block
  ...
  int thread_rank_in_block =
      (threadIdx.z * blockDim.y + threadIdx.y) * blockDim.x +
      threadIdx.x; // rank of thread in block = x + (y * blockDim.x) + (z *
                   // blockDim.x * blockDim.y)
  int block_rank_in_grid =
      (blockIdx.z * gridDim.y + blockIdx.y) * gridDim.x +
      blockIdx.x; // rank of block in grid = x + (y * gridDim.x) + (z *
                  // gridDim.x * gridDim.y)
  int thread_rank_in_grid =
