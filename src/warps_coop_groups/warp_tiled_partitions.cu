// tiled partitions in warps come with instrinsic functions that improve
// efficiency and synchronization btn threads
#include "../cx.h"
#include "../cxtimers.h"
...
__global__ void
reduce6(r_Ptr<float> sums, cr_Ptr<float> data,
        int n) { // r_Ptr<float> is a restricted float pointer the c is const
  ...
  for (int tid = grid.thread_rank(); tid < n; tid += grid.size())
    s[id] += data[tid]; // thread linear addressing loop
  ...
  if (warp.meta_group_rank() == 0) {
    ...
    if (id == 0)
      sums[blockIdx.x] = s[0]; // copy to global memory
  }
}

// showcases intrawarp member functions
__global__ void reduce7(r_Ptr<float> sums, cr_Ptr<float> data, int n) {
  // this kernel reduces data within warps then sums up the warps for the final
  // result works for any block size multiple of 32
  auto grid = cg::this_grid();
  auto block = cg::this_thread_block();
  auto warp = cg::tiled_partition<32>(block);
  ...
  // atomic add then sums up the final result by summing the output of the
  // individual warps
...
int main(int argc, char *argv[]) {
  const int N =
      (argc > 1) ? atoi(argv[1]) : 1 << 24; // Number of elements in the tensor
  ...
  timer2.stop();
  std::cout << "Data generation time: " << timer2.count<cxxtimer::ms>() << " ms"
            << std::endl;
  ...
  reduce6<blockSize>
      <<<gridSize, blockSize>>>(d_sums.data().get(), d_data.data().get(), N);
  ...
  std::cout << "reduce6 kernel execution time: " << timer.count<cxxtimer::ms>()
            << " ms" << std::endl;
  ...
  reduce7<<<gridSize, blockSize>>>(d_sums.data().get(), d_data.data().get(), N);
  ...
  std::cout << "reduce7 kernel execution time: "
            << timer_reduce7.count<cxxtimer::ms>() << " ms" << std::endl;

  // Prepare and time reduce8
  cudaMemset(d_sums.data().get(), 0, gridSize * sizeof(float));
  timer_reduce8.start();

  reduce8<<<gridSize, blockSize>>>(d_sums.data().get(), d_data.data().get(), N);
  ...
  std::cout << "reduce8 kernel execution time: "
            << timer_reduce8.count<cxxtimer::ms>() << " ms" << std::endl;
